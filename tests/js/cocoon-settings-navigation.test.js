...
const bootScriptMatch = topPageSource.match(
  /<script>\s*(\/\/ JavaScriptが有効な場合だけ初期タブを隠し[\s\S]*?)\s*<\/script>/u
);
...
const testMobileSelectSynchronization = async () => {
  ...
  assert.strictEqual( document.getElementById( 'draft-title' ).value, '保存済み値' );
...
const testTimeoutRecovery = async () => {
  ...
  assert.strictEqual( responsiveButton.getAttribute( 'aria-checked' ), 'false' );
  assert.strictEqual(
    document.activeElement,
    tabsButton,
...
const testGeneralSaveFailureFocusRecovery = async () => {
  ...
  await flushMicrotasks();
  assert.strictEqual( status.hidden, false );
  assert.strictEqual( status.textContent, '表示モードを保存できませんでした。' );
  assertViewModeSelection( control, 'tabs' );
  assert.strictEqual(
    document.activeElement,
    tabsButton,
