...
class CocoonSettingsDesignContractTest extends TestCase
{
    ...
    /**
     * 動的タブ定義と実際のページテンプレートから検証用HTMLを生成
     */
    private function renderSettingsPage(bool $custom = false, string $mode = 'tabs'): string
    {
        // グローバル関数やrequire_onceの影響を分離したページ描画
        $program = <<<'PHP'
<?php
use Brain\Monkey\Functions;
require 'tests/bootstrap.php';
Brain\Monkey\setUp();
// 個別フォームの実行を省略し、タブ定義から渡された読み込み先を出力
Functions\when('cocoon_render_settings_tab_content')->alias(static function ($tab_id, $tab) {
    echo implode(',', $tab['forms']);
});
Functions\when('is_amp_enable')->justReturn(true);
Functions\when('is_pwa_enable')->justReturn(true);
Functions\when('current_user_can')->justReturn(false);
Functions\when('submit_button')->justReturn(null);
Functions\when('wp_create_nonce')->justReturn('test-nonce');
Functions\when('get_template_part')->justReturn(null);
Functions\when('cocoon_get_settings_navigation_mode')->justReturn(MERGE_TEST_MODE);
define('HIDDEN_FIELD_NAME', 'cocoon_settings_nonce');
define('SELECT_INDEX_NAME', 'select_index');
$_GET = [];
$_POST = [];
$GLOBALS['_THEME_OPTIONS'] = [];
if (MERGE_TEST_CUSTOM) {
    $GLOBALS['test_mock_apply_filters_callbacks']['cocoon_settings_tabs'] = static function ($tabs) {
        return ['custom' => ['label' => '独自設定']] + $tabs;
    };
...
PHP;
        $program = str_replace(
            ['MERGE_TEST_MODE', 'MERGE_TEST_CUSTOM'],
            [var_export($mode, true), $custom ? 'true' : 'false'],
            $program
        );
        $process = proc_open([PHP_BINARY], [0 => ['pipe', 'r'], 1 => ['pipe', 'w'], 2 => ['pipe', 'w']], $pipes, $this->themeRoot);
    ...
    /**
     * 拡張タブの選択復元とCSS表示が両表示モードで共存することを検証
     */
    public function test_拡張タブと表示モードが同時に維持される(): void
    {
        foreach (['tabs', 'responsive'] as $mode) {
            $html = $this->renderSettingsPage(true, $mode);
            $this->assertStringContainsString('data-navigation-initial-mode="' . $mode . '"', $html);
            $this->assertStringContainsString('id="tab-custom-input" value="tab-custom-input" class="tab-input" type="radio" name="tab-input" checked="checked"', $html);
            $this->assertStringContainsString('id="tab-custom-content"', $html);
            $this->assertStringContainsString('#tab-custom-input:checked ~ #tab-custom-content', $html);
            $this->assertSame(1, substr_count($html, 'checked="checked"'));
            $this->assertSame($mode === 'responsive', str_contains($html, "tabs.classList.add( 'is-navigation-booting' );"));
        }
    }
...
~<input\s+type="hidden"\s+id="<\?php echo esc_attr\(SELECT_INDEX_NAME\); \?>"\s+name="<\?php echo esc_attr\(SELECT_INDEX_NAME\); \?>"\s+value="<\?php echo esc_attr\(\$cocoon_select_index\); \?>">~s
REGEX;

        $this->assertStringContainsString('cocoon_is_settings_post_valid()', $source);
        $this->assertStringContainsString("wp_verify_nonce(wp_unslash(\$_POST[HIDDEN_FIELD_NAME]), 'settings')", $this->readThemeFile('lib/page-settings/settings-tabs.php'));
        $this->assertSame(1, substr_count($form, $nonceInput));
        $this->assertSame(1, preg_match_all($selectIndexPattern, $form));
        $this->assertSame(1, substr_count($form, 'id="<?php echo esc_attr(SELECT_INDEX_NAME); ?>"'));
        $this->assertSame(1, substr_count($form, 'name="<?php echo esc_attr(SELECT_INDEX_NAME); ?>"'));
    ...
    {
        $source = $this->renderSettingsPage();
        ...
        $this->assertStringContainsString('id="tab-skin-input" value="tab-skin-input" class="tab-input" type="radio" name="tab-input" checked="checked"', $source);
    }
    ...
    {
        $topPage = $this->renderSettingsPage();
