        ...
        if ( displayTotal ) {
          ...
          const centerY =
            ( top +
              ...
              ( data.length % 2 === 0
                ? 0
                : getChartJsFontHeight( fontSize ) ) ) /
            2;
          const total = `${ __( '総計:', THEME_NAME ) } ${ arrayValueTotal(
            data
          ) }`;
    ...
    chartInstanceRef.current = new Chart( ctx, {
      ...
      data: {
        labels: labels.map( ( label, index ) =>
          displayLabelValue ? `${ label } ( ${ data[ index ] } )` : label
        ),
        datasets: [
          {
            label: legendText,
      ...
      <InspectorControls>
          ...
          colorSettings={ [
            {
              ...
              onChange: ( newColor ) =>
                setAttributes( { chartColor: newColor } ),
              value: chartColor,
              enableAlpha: false,
            },
            {
            ...
            {
              label:
                __( 'グリッド色', THEME_NAME ) + __( '（※透過）', THEME_NAME ),
              onChange: ( newColor ) =>
        ...
        <PanelBody title={ __( 'チャート設定', THEME_NAME ) }>
          ...
          <ToggleControl
            ...
            onChange={ ( value ) =>
              setAttributes( { displayLabelValue: value } )
            }
          />
          <ToggleControl
            __nextHasNoMarginBottom={ true }
          ...
          <ToggleControl
            ...
            onChange={ ( value ) =>
              setAttributes( { displayAngleLines: value } )
            }
          />
        </PanelBody>
      </InspectorControls>
