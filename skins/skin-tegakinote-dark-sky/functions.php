...
function org_theme_add_editor_styles() {
	$editor_style_url = get_theme_file_uri( '/skins/skin-tegakinote-dark-sky/editor-style.css' );
...
function css_custom() {
	$content_color = get_site_background_color() ?: '#222';
	echo '
	.container,
	...
	body:not(.blog):not(.archive) .main:before,
	.list:before,
	body:not(.blog):not(.archive) .main:after,
	.list:after {
...
function font_css() {
	$style_template = '
		<style>
			.logo-text, .appeal-title, .entry-title, .list-title, .entry-card-title, 　.pagination a, .pagination .current, .article h2, .article h3, .article h4, .article h5, .article h6, .widget h2, .widget-title, .related-entry-main-heading, .comment-title, .footer-title {
				font-family: %s;
			}
		</style>
	';
...
function bg_image_css() {
	$style_template = '
		<style>
			.container, .sidebar-menu-content, .mobile-menu-buttons {
				background-image: %s;
				background-size: %s;
			}
		</style>
	...
	if (get_theme_mod('bg_image_control') === 'bg_grid') {
		$style_image = 'linear-gradient(180deg, rgba(var(--gray-dark-color-rgb),0) 93%, var(--gray-dark-color) 100%),linear-gradient(90deg, rgba(var(--gray-dark-color-rgb),0) 93%, var(--gray-dark-color) 100%)';
		$style_size= '15px 15px';
	} elseif (get_theme_mod('bg_image_control') === 'bg_grid_paper') {
		$style_image = 'linear-gradient(rgba(var(--gray-dark-color-rgb),.7) 1%, rgba(var(--gray-dark-color-rgb),.7) 1%, transparent 1%,   transparent 99%, rgba(var(--gray-dark-color-rgb),.7) 99%, rgba(var(--gray-dark-color-rgb),.7) 100%),  linear-gradient(90deg, rgba(var(--gray-dark-color-rgb),.7) 1%, rgba(var(--gray-dark-color-rgb),.7) 1%, transparent 1%,   transparent 99%, rgba(var(--gray-dark-color-rgb),.7) 99%, rgba(var(--gray-dark-color-rgb),.7) 100%),  linear-gradient(transparent, transparent 25%, rgba(var(--gray-dark-color-rgb),.4) 25%, rgba(var(--gray-dark-color-rgb),.4) 26%, transparent 26%,   transparent 50%, rgba(var(--gray-dark-color-rgb),.4) 50%, rgba(var(--gray-dark-color-rgb),.4) 51%, transparent 51%,   transparent 75%, rgba(var(--gray-dark-color-rgb),.4) 75%, rgba(var(--gray-dark-color-rgb),.4) 76%, transparent 76%, transparent 100%),  linear-gradient(90deg, transparent, transparent 25%, rgba(var(--gray-dark-color-rgb),.4) 25%, rgba(var(--gray-dark-color-rgb),.4) 26%, transparent 26%,   transparent 50%, rgba(var(--gray-dark-color-rgb),.4) 50%, rgba(var(--gray-dark-color-rgb),.4) 51%, transparent 51%,   transparent 75%, rgba(var(--gray-dark-color-rgb),.4) 75%, rgba(var(--gray-dark-color-rgb),.4) 76%, transparent 76%, transparent 100%)';
		$style_size = '80px 80px';
	} elseif (get_theme_mod('bg_image_control') === 'bg_line') {
		$style_image = 'linear-gradient(var(--gray-dark-color) .1px, rgba(var(--gray-dark-color-rgb),0) .05em)';
		$style_size = 'auto 1em';
	} elseif (get_theme_mod('bg_image_control') === 'bg_dot') {
		$style_image = 'radial-gradient(var(--gray-dark-color) 3px, rgba(var(--gray-dark-color-rgb),0) 3px)';
		$style_size= '30px 30px';
	}else {
		$style_image= 'none';
		$style_size= 'unset';
	}
