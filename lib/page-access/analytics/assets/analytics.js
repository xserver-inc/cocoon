...
(function () {
  ...
  function renderClickMap() {
    try {
      ...
      if (!frameDocument) {overlay.classList.add('cocoon-click-map-unavailable'); return;}
      ...
      const areaCounts = {};
      const linkRows = Array.isArray(CA.clickMap) ? CA.clickMap : [];
      frameDocument.querySelectorAll('a[href]').forEach((anchor) => {
        ...
        const row = linkRows.find((item) => { return item.semantic_area === area && Number(item.occurrence_no) === occurrence; });
        if (!row) {return;}
