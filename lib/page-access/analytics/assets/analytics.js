    ...
    inputs.forEach((input) => {
      ...
      function renderDropdown(keyword) {
        ...
        if (filtered.length === 0) {
          ...
          noResults.textContent = i18n.no_matching_items || '';
...
// クリック解析専用のグラフと同一オリジンのプレビュー用バッジを描画します。
(function () {
  'use strict';
  const CA = window.CocoonAnalytics || {};
  ...
  frame.addEventListener('load', () => {
    try {
      ...
      const height = Math.min(20000, Math.max(frameDocument.documentElement.scrollHeight, frameDocument.body ? frameDocument.body.scrollHeight : 0, 800));
