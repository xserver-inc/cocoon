...
(function () {
  ...
  // 時系列グラフのツールチップ用完全日付
  function chartDateTooltipTitle(rows) {
    return function (items) {
      if (!items || !items.length) {return '';}
      const row = rows[items[0].dataIndex];
      return row ? (row.tooltip_label || row.label || row.date) : items[0].label;
    };
  }
  ...
  function formatDateRange(from, to) {
    ...
    return (i18n.date_range_format || '%1$s 〜 %2$s')
      .replace('%1$s', from)
      .replace('%2$s', to);
  }

  // 折れ線グラフの描画点の上限（グラフ幅に対して数ピクセルに1点あれば形状は保たれる）
  const MAX_CHART_POINTS = 200;

  // 点数が上限を超える場合、等間隔のバケットにPVを合計して描画点を減らす
  function downsampleRows(rows, maxPoints) {
    ...
    for (let i = 0; i < rows.length; i += size) {
      ...
      const pv = bucket.reduce((sum, r) => { return sum + (parseInt(r.pv, 10) || 0); }, 0);
      result.push({
        date: first.date,
        pv: pv,
        label: first.label,
        // 1点が複数日分の合計になるため、ツールチップは集約した日付の範囲を示す
        tooltip_label: formatDateRange(first.tooltip_label || first.date, last.tooltip_label || last.date)
      });
    }
    return result;
  }

  // 目盛りラベル1個あたりの目安幅（斜め表示を前提とした概算）
  const TICK_LABEL_WIDTH = 55;

  // グラフ幅から、目盛りを何個おきに表示するかを求める
  function chartTickStep(chart, count) {
    const width = chart && chart.width ? chart.width : 0;
    const maxTicks = Math.max(1, Math.floor(width / TICK_LABEL_WIDTH));
    return Math.max(1, Math.ceil(count / maxTicks));
  }

  // 自前で間引いた目盛り以外はグリッド線も消す
  function chartSparseGridColor(ctx) {
    const count = ctx.scale && ctx.scale.ticks ? ctx.scale.ticks.length : 0;
    return ctx.index % chartTickStep(ctx.chart, count) === 0 ? 'rgba(0, 0, 0, 0.1)' : 'transparent';
  }

  // 表示範囲が複数年にわたるときだけ、年の切り替わり位置と左端に年を併記する関数を返す
  // Chart.js 標準の間引きはラベル生成より後に行われ、年を付けた目盛りが消えてしまうため、
  // 間引き自体を自前で行って表示位置を確定させる（呼び出し側で autoSkip を無効にすること）
  function chartYearAwareTicks(rows) {
    const first = rows && rows.length ? rows[0] : null;
    const last = rows && rows.length ? rows[rows.length - 1] : null;
    const firstYear = first && first.date ? String(first.date).slice(0, 4) : '';
    const lastYear = last && last.date ? String(last.date).slice(0, 4) : '';
    // 単年に収まる範囲では年が自明なため、既定の目盛り表示のままにする
    if (!firstYear || firstYear === lastYear) {return null;}

    return function (value, index, ticks) {
      const step = chartTickStep(this.chart, ticks.length);
      if (index % step !== 0) {return '';}

      const tick = ticks && ticks[index];
      const dataIndex = tick && typeof tick.value === 'number' ? tick.value : index;
      const row = rows[dataIndex];
      if (!row || !row.date) {
        return this && typeof this.getLabelForValue === 'function' ? this.getLabelForValue(value) : value;
      }

      const label = row.label || row.date;
      const year = String(row.date).slice(0, 4);
      const prevTick = index >= step ? ticks[index - step] : null;
      const prevRow = prevTick ? rows[prevTick.value] : null;
      const prevYear = prevRow && prevRow.date ? String(prevRow.date).slice(0, 4) : '';
  ...
  (function () {
    ...
    function updateTrendChart(type) {
      ...
      // 折れ線の日次は長期間だと点数が多すぎるため集約する（週次・月次の棒グラフは元々点数が少ない）
      const list = type === 'daily' ? downsampleRows(source, MAX_CHART_POINTS) : source;

      // Chart.jsに渡すデータ形式の構築（ラベルはPHP側でサイトの日付書式・言語に整形済み）
      const labels = list.map((d) => { return d.label || d.date; });
      const pvData = list.map((d) => { return d.pv; });
      // 点数が多いと曲線補間とアニメーションの描画コストが大きくなる
      const heavy = list.length > MAX_CHART_POINTS;
      ...
      const config = {
        ...
        options: {
          ...
          maintainAspectRatio: false,
          normalized: true,
          // undefined なら Chart.js の既定アニメーションが適用される
          animation: heavy ? false : undefined,
          plugins: {
            legend: { display: false },
            tooltip: {
              callbacks: { title: chartDateTooltipTitle(list) }
            }
          },
          scales: {
      ...
      // 月次・年次のラベルは既に年を含むため、日次・週次だけ年を補う
      if (type === 'daily' || type === 'weekly') {
        const yearTicks = chartYearAwareTicks(list);
        if (yearTicks) {
          config.options.scales.x = {
            ticks: { autoSkip: false, callback: yearTicks },
            grid: { color: chartSparseGridColor }
          };
        }
      }
      ...
      if (type === 'daily') {
        ...
        config.options.plugins.tooltip.callbacks.afterLabel = function (ctx) {
          ...
          if (row && row.days && row.days < 7) {
            return (i18n.partial_week || '') + ': ' + row.days + '/7 ' + (i18n.days || '');
          }
          return '';
        };
        ...
        config.options.plugins.tooltip.callbacks.afterLabel = function (ctx) {
          ...
          if (row && row.days) {
            return (i18n.days_count || '') + ': ' + row.days + ' ' + (i18n.days || '');
          }
          return '';
        };
      }
  ...
  (function () {
    ...
    function updateSummary(filtered) {
      ...
      const firstRow = filtered.length ? filtered[0] : null;
      const lastRow = filtered.length ? filtered[filtered.length - 1] : null;
      const firstDate = firstRow ? (firstRow.tooltip_label || firstRow.date) : '';
      const lastDate = lastRow ? (lastRow.tooltip_label || lastRow.date) : '';
      const dateRange = formatDateRange(firstDate, lastDate);
      const pvUnit = i18n.pv || 'PV';
      summaryEl.textContent = (i18n.shown_period_total || '') + (dateRange ? ' (' + dateRange + ')' : '') +
        ': ' + shownTotal.toLocaleString() + ' ' + pvUnit +
        ' (' + (i18n.list_period_total || '') + ': ' + listTotal.toLocaleString() + ' ' + pvUnit + ' / ' +
        (i18n.all_total || '') + ': ' + allTotal.toLocaleString() + ' ' + pvUnit + ')';
    ...
    function updatePeriodUI() {
      ...
      if (zoomNotice && zoomNoticeText) {
        if (graphPeriodOverride && graphPeriodOverride !== listEquivPeriod()) {
          zoomNoticeText.textContent = (i18n.zoom_notice || '') + ' (' + periodChipLabel(eff) + ')';
    ...
    function renderLifecycleChart(rawRows) {
      ...
      if (!canvas || !rawRows || !rawRows.length) {
        ...
        if (placeholderText) {
          placeholderText.textContent = i18n.no_period_data || '';
        }
      ...
      // 全期間表示では数千日分になるため、描画点を集約する
      const lifecycleData = downsampleRows(rawRows, MAX_CHART_POINTS);
      // 点数が多いと曲線補間とアニメーションの描画コストが大きくなる
      const heavy = lifecycleData.length > MAX_CHART_POINTS;

      // X軸のタイトルを「公開後日数」から「日付」へ変更します
      const scaleX = { title: { display: true, text: i18n.date || 'Date' } };
      const yearTicks = chartYearAwareTicks(lifecycleData);
      if (yearTicks) {
        scaleX.ticks = { autoSkip: false, callback: yearTicks };
        scaleX.grid = { color: chartSparseGridColor };
      }
      ...
      lifecycleChart = new Chart(canvas, {
        type: 'line',
        data: {
          // X軸のラベルはPHP側でサイトの日付書式・言語に整形済み
          labels: lifecycleData.map((d) => { return d.label || d.date; }),
          datasets: [{
            label: i18n.pv || 'PV',
            ...
            fill: true,
            tension: heavy ? 0 : 0.2,
            // 0埋めで点数が増えるため、長期間表示では点を消して見やすさと描画負荷を確保
            pointRadius: lifecycleData.length > 60 ? 0 : 3
          }]
        },
        options: {
          responsive: true,
          ...
          plugins: {
            legend: { display: false },
            tooltip: {
              callbacks: { title: chartDateTooltipTitle(lifecycleData) }
            }
          },
          scales: {
            x: scaleX,
        ...
        if (xhr.status >= 200 && xhr.status < 400) {
          ...
          if (res.success) {
            ...
            if (posts.length === 0 && !append) {
              postsList.innerHTML = '<p style="padding:16px; color:#666; text-align:center;">' + escapeHtml(i18n.no_posts_found || '') + '</p>';
              return;
            }
    ...
    function selectPost(postId) {
      ...
      chartTitle.textContent = i18n.loading || '';
      ...
      xhr.onload = function () {
        if (xhr.status >= 200 && xhr.status < 400) {
          ...
          if (res.success) {
            ...
            chartTitle.textContent = i18n.error_occurred || '';
          }
        } else {
          chartTitle.textContent = i18n.connection_error || '';
        }
      };
      xhr.send();
    }
...
// クリック解析専用のグラフと同一オリジンのプレビュー用バッジの描画
(function () {
  'use strict';
  const CA = window.CocoonAnalytics || {};
  const rows = CA.data && CA.data.click_trend ? CA.data.click_trend : [];
  const canvas = document.getElementById('cocoon-click-trend');
  if (canvas && typeof window.Chart !== 'undefined') {
    new window.Chart(canvas, {
      type: 'bar',
      data: {
        labels: rows.map((row) => { return row.date; }),
        datasets: [
          {label: (CA.i18n && CA.i18n.clicks) || 'Clicks', data: rows.map((row) => { return Number(row.clicks) || 0; }), backgroundColor: 'rgba(34, 113, 177, .65)', yAxisID: 'clicks'},
          {label: (CA.i18n && CA.i18n.ctr) || 'CTR', data: rows.map((row) => { return row.ctr === null ? null : Number(row.ctr) * 100; }), borderColor: '#d63638', backgroundColor: 'transparent', type: 'line', tension: 0.2, yAxisID: 'ctr'}
        ]
      },
      options: {responsive: true, maintainAspectRatio: false, interaction: {mode: 'index', intersect: false}, scales: {clicks: {beginAtZero: true, position: 'left'}, ctr: {beginAtZero: true, position: 'right', grid: {drawOnChartArea: false}, ticks: {callback: (value) => { return value + '%'; }}}}}
    });
  }

  const frame = document.getElementById('cocoon-click-map-frame');
  const overlay = document.getElementById('cocoon-click-map-overlay');
  ...
  (Array.isArray(CA.clickMap) ? CA.clickMap : []).forEach((row) => {
    const key = row.semantic_area + ':' + Number(row.occurrence_no);
    if (!linkRows.has(key)) {linkRows.set(key, row);}
  });
  function documentSize(doc) {
    return [doc.documentElement.clientWidth, doc.documentElement.scrollHeight, doc.body ? doc.body.getBoundingClientRect().height : 0].join(':');
  }
  function disconnectPreview() {
    ...
    if (observedDocument) {
      observedDocument.removeEventListener('load', scheduleRender, true);
      if (observedDocument.defaultView) {observedDocument.defaultView.removeEventListener('pagehide', disconnectPreview);}
    }
    observedDocument = null;
    measuredSize = '';
  }
  function scheduleRender() {
    // 連続したDOM変更を100ms単位でまとめる再描画予約
    if (!renderTimer) {renderTimer = window.setTimeout(() => {renderTimer = 0; renderClickMap();}, 100);}
  }
  function observePreview(doc) {
    if (observedDocument === doc) {return;}
    disconnectPreview();
    observedDocument = doc;
    doc.addEventListener('load', scheduleRender, true);
    doc.defaultView.addEventListener('pagehide', disconnectPreview);
    if (typeof window.ResizeObserver === 'function') {
      resizeObserver = new window.ResizeObserver(() => {
        // iframeの高さ調整そのものによる再描画ループの回避
        if (observedDocument === doc && documentSize(doc) !== measuredSize) {scheduleRender();}
      });
      resizeObserver.observe(doc.documentElement);
      if (doc.body) {resizeObserver.observe(doc.body);}
    }
    if (typeof window.MutationObserver === 'function') {
      mutationObserver = new window.MutationObserver(scheduleRender);
      mutationObserver.observe(doc.documentElement, {childList: true, subtree: true, attributes: true, characterData: true});
    }
    if (doc.fonts && doc.fonts.ready) {
      doc.fonts.ready.then(() => {if (observedDocument === doc) {scheduleRender();}});
    }
  }
  function renderClickMap() {
    try {
      const frameDocument = frame.contentDocument;
      // 遷移前の空ページを除外し、プレビューの読み込み完了後に初期化
      if (!frameDocument) {throw new Error('unavailable');}
      if (frameDocument.URL === 'about:blank' || frameDocument.readyState !== 'complete') {return;}
      observePreview(frameDocument);
      // 短くなった記事も再計測可能な、最小表示高への一時リセット
      frame.style.height = '800px';
      // 計測時の座標と同じ基準を保つため、記事全体の高さを使用
      const height = Math.max(frameDocument.documentElement.scrollHeight, frameDocument.body ? frameDocument.body.scrollHeight : 0, 800);
      frame.style.height = height + 'px';
      overlay.style.height = height + 'px';
      measuredSize = documentSize(frameDocument);
      overlay.classList.remove('cocoon-click-map-unavailable');
      // 再読み込み時の重複を防ぐため、リンク別バッジのみを再作成
      overlay.querySelectorAll('.cocoon-click-link-marker').forEach((badge) => {badge.remove();});
      const areaCounts = {};
      frameDocument.querySelectorAll('a[href]').forEach((anchor) => {
        let area = 'other';
        const override = anchor.closest('[data-cocoon-click-area]');
        if (override) {area = String(override.getAttribute('data-cocoon-click-area') || 'other').toLowerCase();}
        else if (anchor.closest('.toc')) {area = 'toc';}
        else if (anchor.closest('.internal-blogcard-wrap,.external-blogcard-wrap')) {area = 'blogcard';}
        else if (anchor.closest('.related-entry-card-wrap,.related-list')) {area = 'related';}
        else if (anchor.closest('.wp-block-button,.btn-wrap,.ranking-item-link-buttons,.cta-box')) {area = 'cta';}
        else if (anchor.closest('#navi,.navi')) {area = 'navi';}
        else if (anchor.closest('#header-container,#header')) {area = 'header';}
        else if (anchor.closest('#sidebar,.sidebar')) {area = 'sidebar';}
        else if (anchor.closest('#footer,.footer')) {area = 'footer';}
        else if (anchor.closest('.mobile-menu-buttons')) {area = 'mobile_menu';}
        else if (anchor.closest('.entry-content')) {area = 'content';}
        const occurrence = areaCounts[area] || 0;
        areaCounts[area] = occurrence + 1;
        const row = linkRows.get(area + ':' + occurrence);
        if (!row) {return;}
        const rect = anchor.getBoundingClientRect();
        // 非表示リンクも掲載順に含めたうえで、描画対象だけを選別
        const style = frameDocument.defaultView.getComputedStyle(anchor);
        if (!anchor.getClientRects().length || (!rect.width && !rect.height) || style.visibility === 'hidden' || style.visibility === 'collapse') {return;}
        const badge = document.createElement('span');
        badge.className = 'cocoon-click-link-marker';
        badge.style.left = Math.max(0, rect.left + frameDocument.defaultView.scrollX) + 'px';
        badge.style.top = Math.max(0, rect.top + frameDocument.defaultView.scrollY) + 'px';
        badge.textContent = String(Number(row.clicks) || 0);
        badge.title = String(row.anchor_text || row.destination_url || '');
        overlay.appendChild(badge);
      });
    } catch (error) {
      disconnectPreview();
      overlay.querySelectorAll('.cocoon-click-link-marker').forEach((badge) => {badge.remove();});
      overlay.classList.add('cocoon-click-map-unavailable');
    }
  }
  frame.addEventListener('load', renderClickMap);
  window.addEventListener('resize', scheduleRender);
  window.addEventListener('pagehide', disconnectPreview);
  window.addEventListener('pageshow', renderClickMap);
  // スクリプトより先に読み込みを終えたプレビューも初期化の対象
  renderClickMap();
})();
