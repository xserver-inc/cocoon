    ...
    <table class="form-table">
      <tbody>
        ...
        <tr>
          ...
          <td>
            <?php
            generate_checkbox_tag(OP_FEATURED_IMAGE_FROM_TITLE_ENABLE , is_featured_image_from_title_enable(), __( '「タイトルからアイキャッチを生成する」機能を有効にする', THEME_NAME ));
            generate_image_preview_tag('https://wp-cocoon.com/wp-content/uploads/2025/01/featured-image-from-title.png');
            generate_tips_tag(__( 'エディターのアイキャッチ設定エリア内に、「タイトルからアイキャッチを生成する」オプションを追加します。', THEME_NAME ).__( '「公開（保存）」ボタンが押された時だけ画像が生成されます。', THEME_NAME ).get_help_page_tag('https://wp-cocoon.com/automatic-generation-of-featured-image-from-title/'));
            ?>
            <?php
            // 以下の不具合により導入見送り
