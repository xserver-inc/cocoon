          ...
          <td>
            ...
            <ul>
              ...
              foreach ( get_cocoon_sns_share_options() as $sns_key => $sns_option ):
                if ( empty($sns_option['top_key']) ) continue;
                //ラベルはここで翻訳される。フィルターで追加されたSNSでラベル未指定の場合は配列のキー名で代用する
                $sns_label = get_cocoon_sns_share_option_text($sns_option, 'label', $sns_key); ?>
              <li>
                <?php generate_checkbox_tag($sns_option['top_key'], is_cocoon_sns_share_option_visible($sns_option, 'top'), $sns_label); ?>
              </li>
              <?php endforeach; ?>
              <!-- 以下は現在無効化中のシェアボタン（is_top_*_share_button_visible() が常に0を返すため設定一覧には含めない） -->
              <!-- <li>
