...
リリースバージョンの更新は、既存のリリース履歴に合わせて以下の手順で行います。

- `scss/style.scss` と `style.css` のテーマヘッダーにある `Version` を同じ値へ更新する
- コミットの件名は接頭辞を付けず、バージョン番号だけにする（例：`2.9.5`）
- コミット後、同じバージョン番号の軽量タグを接頭辞なしで作成する
- タグがバージョン更新コミットを指していることを確認する

`languages/cocoon.pot` は手動でバージョンだけを書き換えず、翻訳テンプレートを更新するタイミングで `npm run make-pot` を実行します。POTの更新手順は [`scripts/i18n-translation-guide.md`](scripts/i18n-translation-guide.md) を参照してください。
