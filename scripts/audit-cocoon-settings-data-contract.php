...
try {
    // マージ後やCIでも比較可能な、作業ブランチ名に依存しない監査
    ...
    if (!$checkOnly) {
        // Git管理外の保存先が未作成の場合の親ディレクトリ準備
        $reportDirectory = dirname($absoluteOutput);
        if (!is_dir($reportDirectory) && !mkdir($reportDirectory, 0777, true) && !is_dir($reportDirectory)) {
            throw new RuntimeException("報告書の保存先を作成できませんでした: {$outputPath}");
        }
